...
public class CustomPatternLayout extends AbstractStringLayout {
    ...
    private void initRule() {
        try {
            if (CollectionUtils.isEmpty(Log4j2Rule.regularMap)) {
                return;
            }
    ...
    public String hideMarkLog(String logStr) {
        try {
            //1.判断配置文件中是否已经配置需要脱敏字段
            if (StringUtils.isBlank(logStr) || CollectionUtils.isEmpty(KEY_REG_MAP) || CollectionUtils.isEmpty(REG_PATTERN_MAP)) {
                return logStr;
            }
            //2.判断内容是否有需要脱敏的敏感信息
